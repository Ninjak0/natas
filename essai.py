...
def get_english_score(input_bytes):
    ...
    return sum([character_frequencies.get(chr(byte), 0) for byte in
                input_bytes.lower()])
...
def bruteforce_single_char_xor(ciphertext):
    ...
    return sorted(potential_messages, key=lambda x: x['score'], reverse=True)[
        0]
...
def break_repeating_key_xor(ciphertext):
    ...
    for keysize in range(2, 41):
        ...
        chunks = [ciphertext[i:i + keysize] for i in
    ...
    sorted(average_distances, key=lambda x: x['avg distance'])[0]
    possible_plaintext = []
    ...
    for i in range(possible_key_length):
        ...
        print(bruteforce_single_char_xor(block))
...
def calculate_hamming_distance(input_bytes_1, input_bytes_2):
    ...
    for b1, b2 in zip(input_bytes_1, input_bytes_2):
        ...
        # Count the number of differences ('1's) and add to the hamming
        # distance
